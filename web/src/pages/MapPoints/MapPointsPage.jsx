  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'بانتظار المراجعة', count: points.filter(p => p.status === 'pending').length, color: 'text-orange-600' },
          { label: 'معتمدة', count: points.filter(p => p.status === 'approved').length, color: 'text-green-600' },
          { label: 'مرفوضة', count: points.filter(p => p.status === 'rejected').length, color: 'text-red-600' },
        ].map(s => (
      ...
      <div className="flex gap-4 mb-6">
        {['all', 'pending', 'approved', 'rejected'].map((status) => (
          <button key={status} onClick={() => setFilter(status === 'all' ? '' : status)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${((status === 'all' && !filter) || filter === status) ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-50 border'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden h-[500px]">
          <MapContainer center={[24.7136, 46.6753]} zoom={13} style={{ height: '100%', width: '100%' }}>
            ...
            {approvedPoints.filter(p => p.location_lat && p.location_lng).map((point) => (
              <Marker key={point.id} position={[point.location_lat, point.location_lng]}>
                <Popup><div className="text-right"><div className="font-bold">{point.name}</div>{point.note && <div className="text-sm">{point.note}</div>}</div></Popup>
              </Marker>
            ))}
          </MapContainer>
        </div>
