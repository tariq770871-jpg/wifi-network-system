...
export default function ReportsPage() {
  ...
  const technicians = dashboardData?.technicians || {}
  const monthlyTickets = dashboardData?.monthly_tickets || []
  const techPerformance = techData || []

  const pieData = tickets.map(t => ({
    name: t.status,
    value: parseInt(t.count),
    label: { pending: 'قيد الانتظار', assigned: 'معين', in_progress: 'قيد التنفيذ', completed: 'مكتمل', cancelled: 'ملغي' }[t.status] || t.status
  }))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard title="إجمالي البلاغات" value={tickets.reduce((sum, t) => sum + parseInt(t.count), 0)} icon={Ticket} color="blue" />
        <StatCard title="الفنيين النشطين" value={technicians.active || 0} icon={Users} color="green" />
        <StatCard
          title="معدل الإنجاز"
          value={`${techPerformance.length > 0 ? Math.round(techPerformance.reduce((sum, t) => sum + (t.completed / t.total_tasks * 100 || 0), 0) / techPerformance.length) : 0}%`}
          icon={TrendingUp} color="purple"
        />
        <StatCard
          title="أفضل فني"
          value={techPerformance.length > 0 ? techPerformance.reduce((max, t) => t.completed > max.completed ? t : max, techPerformance[0])?.full_name : '-'}
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={monthlyTickets}>
              ...
              <XAxis dataKey="date" tickFormatter={(date) => new Date(date).toLocaleDateString('ar-SA', { day: 'numeric', month: 'short' })} />
              <YAxis />
              <Tooltip formatter={(value) => [value, 'عدد البلاغات']} labelFormatter={(label) => new Date(label).toLocaleDateString('ar-SA')} />
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div className="bg-green-500 h-2 rounded-full transition-all" style={{ width: `${tech.total_tasks > 0 ? (tech.completed / tech.total_tasks * 100) : 0}%` }} />
                      </div>
                      <span className="text-sm font-medium">{tech.total_tasks > 0 ? Math.round(tech.completed / tech.total_tasks * 100) : 0}%</span>
                    </div>
                  </td>
                </tr>
              ))
