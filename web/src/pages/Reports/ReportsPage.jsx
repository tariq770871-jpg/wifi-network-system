...
export default function ReportsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">التقارير والإحصائيات</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">البلاغات اليومية (آخر 30 يوم)</h2>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-bold mb-4 text-gray-900 dark:text-white">توزيع حالات البلاغات</h2>
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-4">
            {pieData.map((entry, index) => (
              <div key={entry.name} className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-900 dark:text-gray-300">{entry.label} ({entry.value})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-x-auto">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700"><h2 className="text-lg font-bold text-gray-900 dark:text-white">أداء الفنيين</h2></div>
        <table className="w-full min-w-[700px]">
          <thead className="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">الفني</th>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">إجمالي المهام</th>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">مكتملة</th>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">قيد التنفيذ</th>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">معلقة</th>
              <th className="px-4 py-3 text-right text-sm font-medium text-gray-500 dark:text-gray-400">نسبة الإنجاز</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {techLoading ? (
              ...
              <tr><td colSpan={6} className="text-center py-8 text-gray-500 dark:text-gray-400">لا توجد بيانات</td></tr>
            ) : (
              techPerformance.map((tech) => (
                <tr key={tech.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{tech.full_name}</td>
                  <td className="px-4 py-3 text-gray-900 dark:text-white">{tech.total_tasks || 0}</td>
                  <td className="px-4 py-3 text-green-600 dark:text-green-400">{tech.completed || 0}</td>
                  <td className="px-4 py-3 text-purple-600 dark:text-purple-400">{tech.in_progress || 0}</td>
                  <td className="px-4 py-3 text-orange-600 dark:text-orange-400">{tech.pending || 0}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-900 dark:text-white">{(tech.total_tasks || 0) > 0 ? Math.round((tech.completed || 0) / tech.total_tasks * 100) : 0}%</span>
                    </div>
                  </td>
                </tr>
              ))
